...
import os
from make_pairs import create_pairs
...
def model(inputs):
    conv_1 = Conv2D(64, (10, 10), activation='relu', kernel_initializer=W_init, kernel_regularizer=l2(2e-4))(inputs)
    # conv_1 = Conv2D(64, (10, 10), activation='relu')(inputs)
    pool_1 = MaxPooling2D()(conv_1)

    conv_2 = Conv2D(128,(7,7),activation='relu', kernel_regularizer=l2(2e-4),kernel_initializer=W_init,bias_initializer=b_init)(pool_1)
    # conv_2 = Conv2D(128,(7,7),activation='relu')(pool_1)
    pool_2 = MaxPooling2D()(conv_2)

    conv_3 = Conv2D(128,(4,4),activation='relu',kernel_initializer=W_init,kernel_regularizer=l2(2e-4),bias_initializer=b_init)(pool_2)
    pool_3 = MaxPooling2D()(conv_3)

    conv_4 = Conv2D(256,(4,4),activation='relu',kernel_initializer=W_init,kernel_regularizer=l2(2e-4),bias_initializer=b_init)(pool_3)


    f = Flatten()(conv_4)

    dense_1 = Dense(4096, activation="sigmoid",kernel_regularizer=l2(1e-3),kernel_initializer=W_init,bias_initializer=b_init)
    return dense_1
...
prediction = Dense(16,activation='sigmoid',bias_initializer=b_init)(both)
siamese_model = Model(input=[left_input,right_input],output=prediction)
#optimizer = SGD(0.0004,momentum=0.6,nesterov=True,decay=0.0003)
optimizer = Adam(0.001)
#//TODO: get layerwise learning rates and momentum annealing scheme described in paperworking
siamese_model.compile(loss="sparse_categorical_crossentropy",optimizer=optimizer, metrics=[f1, 'accuracy'])
...
loss_history = siamese_model.fit_generator(siam_gen(train_groups), 
    validation_data=([valid_a, valid_b], valid_sim),steps_per_epoch=100,epochs=10, verbose=True)
# siamese_model.fit([tr_pairs[:, 0], tr_pairs[:, 1]], tr_y,
          # validation_data=([te_pairs[:, 0], te_pairs[:, 1]], te_y),
          # batch_size=32,
          # epochs=10)

siamese_model.save_weights('sia_1.h5')
